...
export const ToolCard = ({ item }: ToolCardProps) => {
	const iconSrc =
		item?.icon && item?.icon.length > 0 ? item?.icon[0].url : "";
	...
	return (
		<>
			<Card className="p-2">
				...
				>
					<span className="flex flex-col gap-3 h-full">
						<span className="flex items-center gap-3">
							<CardTitle className="mt-1">{item?.name}</CardTitle>
							{category && (
								<Link
									className="transition-all group-hover:text-emerald-200 ml-auto text-xs text-slate-400 font-light lowercase"
									key={category?.slug}
						...
						<p className="flex justify-start items-start gap-2 flex-wrap mt-auto">
							{item?.tag?.map((tag) => (
								<Link
									className="transition-all text-xs text-slate-400 group-hover:text-slate-200 font-light lowercase"
									key={tag?.slug}
									href={`/tag/${tag?.slug}`}
								>
									#{tag?.slug}
